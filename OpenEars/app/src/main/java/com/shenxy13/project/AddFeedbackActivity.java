...
public class AddFeedbackActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        Toolbar appbar = findViewById(R.id.addfeedbackbar);
    ...
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == RESULT_LOAD_IMAGE && resultCode == RESULT_OK && data != null) {
            ...
            TableLayout imgs = findViewById(R.id.addfeedbackimages);
    ...
    public void submitFeedback(View view) {
        ...
        EditText text = findViewById(R.id.addfeedbacktext);
        ...
        RuntimeDatastore.database.getReference().child("organisations").child(RuntimeDatastore.currentOrganisation).child("feedback").child(RuntimeDatastore.currentPost).child("children").child(key).setValue(childdata).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    RuntimeDatastore.database.getReference().child("organisations").child(RuntimeDatastore.currentOrganisation).child("feedback").child(key).setValue(postdata).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                RuntimeDatastore.database.getReference().child("users").child(RuntimeDatastore.auth.getUid()).child("organisations").child(RuntimeDatastore.currentOrganisation).child("posts").child("f" + key).setValue(ptext).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
